...
public class DlmsServerImpl {
  ...
  //-- constructors    
    public DlmsServerImpl(String name, String inetAddress, int port, HashMap<String,String[]> libServersUdpInfo, DlmsLogger serverLogger){
    	this.setName(name);
    	this.setPort(port);   
  ...
  //-- methods
  	...
  	public String borrowItem(String userID, String itemID, int numberOfDays){
  		try {
  			...
  			if(result.equalsIgnoreCase("")) {
  				return borrowItemCore(userID, itemID, numberOfDays, false);
  			}
  			else
  				return result;
  	 ...
  	 */
  	private String borrowItemCore(String userID, String itemID, int numberOfDays, boolean isExchange){
  		try {
  			String request, result;
  			...
  			if(itemLibName.equalsIgnoreCase(this.getName())) {
  				...
  				if(getItemList().get(itemID) != null) {
  					if(getItemList().get(itemID).getQuantity() > 0 && getItemList().atomicDecreaseQuantity(itemID, 1)) {
  						...
  						result = "[wait] the item with id (" + itemID + ") is not available now to be borrowed";
  						getServerLogger().log((userID + " request result = " + result), true);
  	...
  	public String joinToItemWaitList(String userID, String itemID, int numberOfDays){
  		try {
  			String result;
  			result = clientValidation(userID, "addWaitQueue");
  			if(result.equalsIgnoreCase("")) {
...
//  						}	
  				...
  				String[] param = {userID,itemID, String.valueOf(numberOfDays)};
  				String[] otherLibUdpInfo =getLibServersUdpInfo().get(itemLibName);
  				result = sendUdpRequest("addWaitQueue", param, otherLibUdpInfo);	
  	...
  	private String exchangeItemCore(String userID, String newItemID, String oldItemID){
  		try {
  			...
  			boolean inLibExchange = oldItemLibName.equalsIgnoreCase(newItemLibName);
  				...
  				if(getBorrowList().get(oldItemID) != null) {					
  					for(Borrower brw : getBorrowList().get(oldItemID)) {
  						if(brw.getUserId().equalsIgnoreCase(userID)) {		
  							...
  							//-- check if new item belongs to this library
  							if(newItemLibName.equalsIgnoreCase(this.getName())) {
  								result = borrowItemCore(userID, newItemID, brw.getNumberOfDays(),inLibExchange);
  							}
  							else {
  							...
  							//-- the borrowing of new item failed
  								...
  								if(result.contains("[wait]")) {
  									...
  									result = result.replace("[wait]", "[failed]");
  								}
  			...
  			if(newItemLibName.equalsIgnoreCase(this.getName())) {								
  				...
  				result = borrowItemCore(userID, newItemID, 1, inLibExchange);
  				
  				if(result.contains("[succeed]")) {					
  					// return request for old item
  					String[] param = {userID,oldItemID};
  					String[] otherLibUdpInfo =getLibServersUdpInfo().get(oldItemLibName);
  				    //-- check if old item belongs to this library
					if(oldItemLibName.equalsIgnoreCase(this.getName())) {
						result =  returnItemCore(userID, oldItemID);
					}
					else {
  	...
  	private boolean clientMethodAccessCheck(String clientId, String invokedMethod) {
  		try {
  			...
  			if(invokedMethod.equalsIgnoreCase("borrowItem") || invokedMethod.equalsIgnoreCase("findItem") || invokedMethod.equalsIgnoreCase("returnItem") || 
  			   invokedMethod.equalsIgnoreCase("addWaitQueue") || invokedMethod.equalsIgnoreCase("leaveItemWaitList") || invokedMethod.equalsIgnoreCase("exchangeItem")) {
  	...
  	private void listenToUdpSocket() {
  		...
  		try {
  			...
  			while (true) {
  				...
  				//distinguished between internal server-server udp messaging and external udp messaging
  				if(!requestString.contains("_")) {
	  				dataPresentation.udpMessageUnPacking(requestString);	  				
	  				...
	  				reqResult = processUdpRequest(dataPresentation.getRequest(), dataPresentation.getRequestParam(), false);
	  				...
	  				dataAdaptor.udpMessageUnPacking(requestString);	  				
	  				String udpRequest = dataAdaptor.getRequest() + " with these params " + Arrays.toString(dataAdaptor.getRequestParam())+ " from < replica manager >";
	  				getServerLogger().log((" received udp request : " + udpRequest), true);	  				
	  				reqResult = processUdpRequest(dataAdaptor.getRequest(), dataAdaptor.getRequestParam(), true);
	  				getServerLogger().log(("sent udp response : " + reqResult + " to < front end>"), true);
	  				byte[] replyBuffer = new byte[reqResult.length()];
	  				replyBuffer = (dataAdaptor.getSequenceID() + "_" +reqResult).getBytes();
	  				DatagramPacket reply = new DatagramPacket(replyBuffer, reqResult.length(), InetAddress.getByName(dataAdaptor.getFEUdpInetAddress()), Integer.valueOf(dataAdaptor.getFEUdpPort()));
	  				udpSocket.send(reply);
  				}
  			}
  	 ...
  	 * @param isExternalUDPReq true if the request is from cluster outside like sequencer else false
  	 * @return result of the UDP request produced by the library server
  	 */
  	private String processUdpRequest(String request, String[] requestParam, boolean isExternalUDPReq) {
  		try {
  			String result = "";
  			if(!isExternalUDPReq) {
	  			if(request.equalsIgnoreCase("borrowItem")) {
	  				result =  borrowItemCore(requestParam[0], requestParam[1], Integer.parseInt(requestParam[2]), false);
	  			}
	  			...
	  			else if(request.equalsIgnoreCase("addWaitQueue")) {
	  			...
	  			if(request.equalsIgnoreCase("borrowItem")) {
	  				result =  borrowItem(requestParam[0], requestParam[1], Integer.parseInt(requestParam[2]));
	  			}
	  			else if(request.equalsIgnoreCase("findItem")) {
	  				result = findItem(requestParam[0], requestParam[1]);
	  			}
	  			else if(request.equalsIgnoreCase("returnItem")) {
	  				result = returnItem(requestParam[0], requestParam[1]);
	  			}
	  			
	  			else if(request.equalsIgnoreCase("addWaitQueue")) {
	  				result = joinToItemWaitList(requestParam[0], requestParam[1], Integer.parseInt(requestParam[2]));
	  			}
	  			
	  			else if(request.equalsIgnoreCase("leaveItemWaitList")) {
	  				result = leaveItemWaitList(requestParam[0], requestParam[1]);
	  			}
	  			
	  			else if(request.equalsIgnoreCase("exchangeItem")) {
	  				result = exchangeItem(requestParam[0], requestParam[1], requestParam[2]);
	  			}
	  			
	  			else if(request.equalsIgnoreCase("addItem")) {
	  				result = addItem(requestParam[0], requestParam[1], requestParam[2], Integer.valueOf(requestParam[3]));
	  			}
	  			
	  			else if(request.equalsIgnoreCase("removeItem")) {
	  				result = removeItem(requestParam[0], requestParam[1], Integer.valueOf(requestParam[2]));
	  			}
	  			
	  			else if(request.equalsIgnoreCase("listItemAvailability")) {
	  				result = listItemAvailability(requestParam[0]);
	  			}
  ...
  //-- fields accessor
  	...
  	class FindItemThread implements Runnable{
  		...
  		int resultNo;
  		String[] result;
  		DlmsServerImpl server;
  		CountDownLatch latch;
  		
  		public FindItemThread(String[] param, String[] toServerUdpInfo, String[] findResult, DlmsServerImpl server , CountDownLatch latch) {
